...
The maintainer checkout can retain `.openai/hosting.json` and the Sites build workflow. When that file is absent, local development and ordinary builds use account-independent defaults. `build:standalone` always excludes it. Sites publishing uses the maintainer's selected project, access policy and runtime bindings; authenticated hosted checks still need their own recorded results.
