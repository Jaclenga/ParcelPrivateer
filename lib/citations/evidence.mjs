...
export function findApplicationConflicts(hits, allSources, allChunks, now = new Date()) {
  ...
  for (const chunk of allChunks) {
    ...
    if (!status) continue;
    const values = groups.get(topic) ?? [];
    values.push({ source, chunk, applicationStatus: status });
    groups.set(topic, values);
  }
  for (const [topic, values] of groups) {
    if (new Set(values.map(value => value.applicationStatus)).size > 1) return { topic, hits: [values.find(value => value.applicationStatus === 'open'), values.find(value => value.applicationStatus === 'closed')] };
  }
  return null;
}
