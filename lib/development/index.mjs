...
import { haversineMeters, validPoint, withinServiceRegion } from '../geospatial/index.mjs';
...
export function normalizeDevelopmentCsv(text, settings = config) {
  ...
  const records = rows.flatMap(row => {
    ...
    if (!validPoint(point) || !withinServiceRegion(point) || !id || !recordId || seen.has(id)) { excludedRows++; return []; }
    seen.add(id);
    const statusDate = sourceDate(row.status_date);
    const updatedDate = sourceDate(row.last_updated);
    const createdDate = sourceDate(row.record_created_date);
    const date = statusDate ?? updatedDate ?? createdDate;
    ...
    return [{
      ...
      date, dateType: statusDate ? 'source status date' : updatedDate ? 'source last-updated date' : createdDate ? 'source record-created date' : 'date not supplied',
...
export function createDevelopmentClient({ fetcher = fetch, settings = config, now = () => new Date() } = {}) {
  let cache;
  let inFlight;
  ...
  async function getNearbyDevelopment(point, radiusMeters = settings.default_radius_meters) {
    ...
    if (!validPoint(point) || typeof radiusMeters !== 'number' || !Number.isFinite(radiusMeters) || radiusMeters < settings.min_radius_meters || radiusMeters > settings.max_radius_meters) {
      return { ...base, status: 'invalid_input', message: `Select a location and a distance between ${settings.min_radius_meters} and ${settings.max_radius_meters} meters.`, records: [], retrievedAt: null, totalMatches: 0, activityByYear: [] };
    }
    if (!withinServiceRegion(point)) return { ...base, status: 'missing_coverage', message: 'This location is outside the Tampa area covered by this development lookup.', records: [], retrievedAt: null, totalMatches: 0, activityByYear: [] };
    try {
      ...
      return { ...base, status: 'unavailable', message: 'The independent development-record snapshot could not be loaded or its format changed. Open the source project to review its published records.', records: [], retrievedAt: null, totalMatches: 0, activityByYear: [] };
    }
  }
