...
export function routeQuestion(question, { jurisdictionId = 'tampa-bay' } = {}) {
  ...
  const outOfScope = (Math.max(...Object.values(scores)) === 0 && !startsWithAddress && !/\b(help|start|lost|confused|go|apply|tampa|hillsborough|petersburg|pete|clearwater|pinellas|pasco)\b/.test(text)) || /\b(pizza|song|recipe|sports score|football|weather|stock price)\b/.test(text);
  ...
  return { category, subjectCategory: topic, needsAddress, hasAddress, outOfScope, outsideCoverage, officialJudgment, normalized: text, ...jurisdiction };
}
